...
async def create_welcome_image(member):
    ...
    # --- Xử lý độ trong suốt của avatar ---
    ...
    # Nếu avatar không có kênh alpha (ví dụ: JPG), alpha_channel sẽ là một ảnh trắng (hoàn toàn đục)
    try:
    ...
    # Vùng trong suốt sẽ là vùng có alpha thấp từ avatar gốc VÀ bên ngoài hình tròn
    # Vùng opaque sẽ là vùng có alpha cao từ avatar gốc VÀ bên trong hình tròn
    combined_alpha_mask = Image.composite(avatar_circular_mask, Image.new('L', avatar_circular_mask.size, 0), original_alpha)

    # Tạo một lớp nền màu mờ cho những vùng trong suốt của avatar (nằm trong hình tròn)
    # Màu này sẽ có màu của stroke_color nhưng với độ trong suốt thấp (ví dụ: 150/255)

    # Tạo một màu có độ trong suốt cho background của avatar
    # alpha_fill_color_value = 150 # Độ trong suốt 0-255 (150 là khoảng 60% đục)
    # avatar_fill_color = (*stroke_color_rgb, alpha_fill_color_value)

    # Cách tốt hơn: Tính toán alpha dựa trên độ sáng của stroke color
    # Nếu stroke_color quá sáng, giảm alpha để không quá chói.
    # Nếu stroke_color tối, tăng alpha để làm rõ hơn.
    _, _, stroke_l = rgb_to_hsl(*stroke_color_rgb)

    # Điều chỉnh alpha dựa trên độ sáng của stroke_color
    # Ví dụ: nếu stroke_l = 0.5 (trung bình), alpha = 150.
    # Nếu stroke_l = 0.8 (sáng), alpha sẽ giảm (ví dụ: 100).
    # Nếu stroke_l = 0.2 (tối), alpha sẽ tăng (ví dụ: 200).
    # Công thức: alpha = 255 - (stroke_l * 150) - 50 (giới hạn min 50, max 200)
...
# --- Để bot luôn online trên Replit ---
...
def run():
  # Chỉ lắng nghe trên localhost nếu không cần truy cập từ bên ngoài
  # app.run(host='127.0.0.1', port=8080)
    app.run(host='0.0.0.0', port=8080) # Dùng 0.0.0.0 để Replit có thể truy cập
